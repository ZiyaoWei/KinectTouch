...
PlayedNote::PlayedNote(const Note& note, unsigned long sampleRate, double maxAmp):note(note) {
  double * amp;
  noteLength = noteSeconds * sampleRate;
  wave = (jack_default_audio_sample_t *)malloc(noteLength * sizeof(jack_default_audio_sample_t));
  ...
  jack_default_audio_sample_t scaleLow = PI * note.getFrequency() / sampleRate;
  jack_default_audio_sample_t scaleHigh = 4 * PI * note.getFrequency() / sampleRate;
  jack_default_audio_sample_t scaleSharp = 8 * PI * note.getFrequency() / sampleRate;
  amp = (double *)malloc(noteLength * sizeof(double));
  std::cout<<attackLength<<" "<<decayLength<<" "<<scale<<std::endl;
  ...
  for (int i = (int)noteLength - decayLength; i < (int)noteLength; i++) {
    amp[i] = (-1) * maxAmp * ((i - (double)noteLength) / ((double)decayLength)) * (i - (double)noteLength) / ((double)decayLength);
  }
